...
type W = {
  bag_id: number; bag_index: number; route_id: number; weight_kg: number; volume_l: number;
  volume_limit_l: number; discount_applied: boolean;
  fill_weight_pct: number; fill_volume_pct: number;
};
export default function WeightsPage() {
  ...
  return (<>
    ...
    <tbody>{rows.map(w => <tr key={w.bag_id}><td>{w.bag_index}</td><td>{w.route_id}</td><td className="mono">{w.weight_kg}kg</td>
      ...
      <td><div className="fill"><span style={{ width: `${Math.min(100, w.fill_volume_pct)}%` }} /></div><span className="mono">{w.fill_volume_pct}%</span>
        <span className="mono"> {w.volume_l}/{w.volume_limit_l}L{w.discount_applied ? "（折扣上限）" : ""}</span></td>
    </tr>)}</tbody></table>
  </>);
}
