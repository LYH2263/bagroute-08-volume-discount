...
export default function BagsPage() {
  ...
  return (<>
    <h2>袋明细</h2>
    <table className="table"><thead><tr><th>路线</th><th>袋号</th><th>重量</th><th>体积 / 上限</th><th>订户</th></tr></thead>
    <tbody>{rows.map(b => <tr key={b.id}><td>{b.route_id}</td><td>{b.bag_index}</td><td className="mono">{b.weight_kg}</td>
      <td className="mono">{b.volume_l} / {b.volume_limit_l}L{b.discount_applied ? "（折扣）" : ""}</td>
