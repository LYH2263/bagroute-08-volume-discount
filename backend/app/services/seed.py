...
def seed_if_empty(db: Session) -> None:
    ...
    # 滨江晚线：两站点各 6L，原上限 12L 可同袋；5 折后上限 6L 必须分袋（重量 4kg 远低于 10kg，不触发分袋）
    r3 = DeliveryRoute(
        name="滨江晚线",
        max_weight_kg=10.0,
        max_volume_l=12.0,
    ...
    db.add_all(
        [
            ...
            SubscriberStop(route_id=r3.id, seq=1, name="江景公寓正门", weight_kg=2.0, volume_l=6.0),
            SubscriberStop(route_id=r3.id, seq=2, name="临江茶馆", weight_kg=2.0, volume_l=6.0),
        ]
    )
    db.commit()
